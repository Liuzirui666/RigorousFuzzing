...
def generate_json_summary(coverage_binary,
    ...
    import logging
    # 防御性检查：只要有一个是 None，就记录并跳过
    if coverage_binary is None or profdata_file is None:
        logging.error("CRITICAL: Skipping json summary due to None values. Binary: %s, Profdata: %s", 
                      coverage_binary, profdata_file)
        # 手动创建一个空的 JSON 文件，防止后续处理读取时找不到文件报错
    ...
    with open(output_file, 'w', encoding='utf-8') as dst_file:
        try:
            ...
            logging.error("Failed to execute llvm-cov command: %s", e)
            return None
